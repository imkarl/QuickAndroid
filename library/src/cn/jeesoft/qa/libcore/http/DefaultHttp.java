...
public class DefaultHttp implements QAHttp {
    ...
    public <T> void load(Context context, QAHttpMethod method,
        ...
        getHttpClient().load(context, method, url, params, parser, listener);
    }
    ...
    public static <T> Class<T> getResultType(QAHttpCallback<T> listener) {
        final Class<T> type;
        try {
            type = QACore.qclass.getGenericInterfaces(listener.getClass())[0];
        } catch (Exception e) {
